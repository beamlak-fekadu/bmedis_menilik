...
Last updated: 2026-05-21 (Phase 1 system-truth and workflow-blocker sweep. Migration 00073 adds narrow "Self insert" RLS policies to `notifications` and `notification_events` so viewer + dept-scoped roles can run the in-app self-test (NOTIF-03). Public/sw.js rewritten: navigation responses are now cached in PAGES_CACHE so previously visited routes load offline (OFF-01); cache version bumped to v2, auth/api/RSC excluded, opaque/redirected/error responses rejected. src/lib/offline/sync-engine.ts: every `recordSyncEvent` call now returns SyncEventOutcome; failures annotate the local queue record with `evidence_write_failed`/`evidence_write_error` instead of being silently swallowed (OFF-03); WO completion-draft messaging made honest — toast and modal explicitly state the WO remains open until reconnect + Confirm Completion (OFF-02). Notification rules: rule_workOrderAssigned reads `technician_profile_id` OR `assigned_to` so both emit conventions resolve (NOTIF-01); createWorkOrderAction now emits both keys. Calibration emits include `requested_by`, `request_id`, `request_number`, and `record_id` so requester notifications fire and deep links open exact records (NOTIF-02). PM completion (createPMCompletionAction) is now transactional: pm_completions inserted, pm_schedules updated with compensating rollback on failure, pm_plans / equipment_assets / analytics refresh failures captured into a `warnings[]` array; UI surfaces every warning as a separate toast (PM-01). SHAPE-01 sweep: maintenance/PM/calibration/installation/disposal/specification/spare-parts/procurement/equipment/users UPDATE paths use .maybeSingle() + interpretMissingMutationResult helper; PGRST116 no longer leaks. EMBED-01 sweep: maintenance.service, calendar-data, command-center-data, reports.service, decision-support.service, qr-context.service, developer-lab.service now use explicit FK hints (`profiles!work_orders_assigned_to_fkey`, `profiles!procurement_requests_requested_by_fkey`, `profiles!equipment_qr_scans_scanned_by_fkey`, etc.). ANALYTICS-01: `_shared.runAnalyticsRefreshOrWarn` helper plus inline try/catch replace all silent `.catch(() => undefined)` on recomputeAssetAnalytics — failures now audit `*.analytics_refresh_failed` and propagate via `data.analytics_refresh_warning`. Calibration request access for department_head/department_user verified working (migrations 00071/00072 already in place); 12 new locking tests. New Developer Lab section: Phase 1 RLS invariants probe `auth_profile_department_id`, `can_create_calibration_request_for_asset`, `is_dept_scoped_role`, `offline_sync_events` Phase 3 columns, and `notifications` shape — any missing migration becomes a top-line environmentWarning. `documents/phase1-rls-validation-matrix.md` lists the deployed-env browser sign-off steps. New regression tests (107 added; suite now 522/522): service-worker-policy, offline-honesty, notification-payload-contract, pm-completion-truth, shape-01-no-coercion, analytics-01-no-swallow, embed-fk-hints, calibration-access. Suite 522/522, lint clean, tsc clean, build 60+ routes. Manual browser validation NOT performed — see documents/phase1-rls-validation-matrix.md for the steps an evaluator must run on the deployed Vercel preview.

Earlier (workflow-truth fix): completed corrective work orders now ALWAYS produce a linked maintenance_events row. **Follow-up: migration 00068** added bme_head to maintenance_events INSERT/UPDATE RLS — without it the writer surfaced "new row violates row-level security policy for table maintenance_events" for any bme_head completing a WO (the work_orders UPDATE itself succeeded thanks to 00067, but the follow-up event insert was denied because 00012/00026 only allowed admin/technician/developer/store_user on maintenance_events). The action layer now translates the raw PostgREST RLS error into an actionable user message that names the table and the fix (apply migration 00068), and preserves the raw cause in audit_logs.details.postgrest_error. Root cause of the original event-not-showing-up bug: updateWorkOrderAction's `hasEvidence` gate only ran the event writer when the caller supplied explicit reliability fields, so a technician clearing the prefilled modal fields silently completed the WO without any evidence and only saw a warning toast. Additionally, the Work Order Detail page queried `maintenance_events` by `asset_id` and never refreshed after completion, so even existing events wouldn't appear under "Maintenance Events" without a manual reload. Fix: pure helper `src/utils/maintenance/completion-evidence.ts → deriveReliabilityEvidence` returns a full evidence shape for every corrective completion, deriving missing fields from the WO's started_at / completed_at / created_at + actual_hours and the originating request's created_at. updateWorkOrderAction ALWAYS writes the event for corrective work and uses an idempotent upsert keyed by `completion_date IS NOT NULL` on the WO row (re-completion UPDATEs, not duplicates). New `getMaintenanceEventsByWorkOrderId` service helper. WO detail page now: queries events by work_order_id, reloads events after completion, has a new "Action taken / repair summary" textarea (persists to both work_orders.action_taken and maintenance_events.action_taken), surfaces an honest amber banner when a completed corrective WO has zero linked events, and links to asset-wide history under /equipment/[id]. `reliability_evidence_warning` is now only emitted on hard write failure (RLS denial / constraint violation), not on missing input. No DB migration. New audit event: `maintenance_event.updated_from_work_order_completion` (re-completion update path). 13 new tests at `src/utils/maintenance/__tests__/completion-evidence.test.ts`. Suite: 415/415, lint clean, tsc clean, build 54/54 routes.
...
- [updateWorkOrderAction completion branch](src/actions/maintenance.actions.ts):
  ...
  - Accepts optional `repair_duration_hours`, `downtime_start`,
    `downtime_end`, `failure_date`, and action summary fields from the
    completion payload. For corrective work orders it always inserts or
    updates one linked `maintenance_events` completion row, deriving missing
    values from the WO/request timestamps where possible; the trigger from
    00061 then derives the matching `downtime_logs` row that
    `fn_compute_mtbf` reads.
  - `reliability_evidence_warning` is returned only when the
    `maintenance_events` write fails (RLS, constraint, or no returned row),
    not when optional fields were left blank.
  - Audits `maintenance_event.created_from_work_order_completion` or
    `maintenance_event.updated_from_work_order_completion` on success,
    `work_order.reliability_evidence_write_failed` on failure.
...
3. Manual validation:
   ...
   - Complete a corrective WO without manually filling reliability fields →
     one linked `maintenance_events` row still exists with derived evidence;
     no missing-input warning is emitted.
